        ...
        public void onCreate(SQLiteDatabase db) {
            String sql = String.format(
                    "CREATE TABLE IF NOT EXISTS %s "+
                            ...
                            "   %s TEXT, "+
                            "   %s TEXT "+
                            ")", MEMBERS, SEQ, NAME, PASSWD, EMAIL, PHONE, ADDR, PHOTO);
            Log.d("실행할 쿼리 : ", sql);
            db.execSQL(sql);
            String[] names = {"강동원", "윤아", "임수정", "박보검", "송중기"};
            String[] emails = {"kdw@test.com","yoona@test.com","Isj@test.com","pbk@test.com","songjk@test.com"};
            for(int i=0; i<names.length; i++){
                db.execSQL(String.format("INSERT INTO %s " +
                        ...
                        " '%s' ,"+
                        " '%s' ,"+
                        " '%s' )", MEMBERS, NAME, PASSWD, EMAIL, PHONE, ADDR, PHOTO, names[i], "1", emails[i]+"@test.com",
                        "010-1234-5678"+(i+1), "백범로" + (i+1) + "길", "photo_"+(i+1)));
            }
            Log.d("INSERT 쿼리", "SECCESS !!");
        }
        @Override
